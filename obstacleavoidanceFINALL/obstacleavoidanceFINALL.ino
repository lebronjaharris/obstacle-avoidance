...
const int threshold = 40;    // Obstacle distance threshold in cm 
...
delay(500); 
    if (distance > threshold) { 
        forward(80); // Move forward if no obstacle 
    } else { 
    ...
    if (distance > threshold) { 
            turnLeft(110);    // Turn left if no obstacle on the left 
            myservo.write(90); // Reset servo to center 
            delay(500); 
            forward(80); 
            } 
             else {    // Move
              backward(80);  // Move backward 
                delay(1000); 
                turnRight(200);  // Reverse for 1 second 
                delay(1000);
